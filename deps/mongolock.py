...
class MongoLock(object):
    ...
    def release(self, key, owner):
        ...
        status = self.collection.find_one_and_update(
            filter={'_id': ObjectId(key), 'owner': owner},
            update={'$set': {'locked': False, 'owner': None,
                             'created': None, 'expire': None}}
        )
    ...
    def touch(self, key, owner, expire=None):
        ...
        lock = self.collection.find_one({'_id': ObjectId(key), 'owner': owner})
        if not lock:
            raise MongoLockException(
                u'Can\'t find lock for {key}: {owner}'.format(key=key, owner=owner))
        if not lock['expire']:
            return
        if not expire:
            raise MongoLockException(
                u'Can\'t touch lock without expire for {0}: {1}'.format(key, owner))
        expire = datetime.utcnow() + timedelta(seconds=expire)
        self.collection.update_one(
            {'_id': ObjectId(key), 'owner': owner},
            {'$set': {'expire': expire}}
        )
    ...
    def _try_get_lock(self, key, owner, expire):
        ...
        result = self.collection.update_one(
            {
                '$or': [
                    {'_id': ObjectId(key), 'locked': False},
                    {'_id': ObjectId(key), 'expire': {'$lt': dtnow}},
                ]
            },
            {'$set': {
                'locked': True,
